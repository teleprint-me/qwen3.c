...
void read_checkpoint(
    ...
    if (!file) {
        fprintf(stderr, "Couldn't open checkpoint %s\n", checkpoint);
        exit(EXIT_FAILURE);
    }
...
void load_prompt_template(
    char* checkpoint_path, char* out_template, int with_system_prompt, int enable_thinking
) {
    ...
    if (with_system_prompt) {
        strcat(
            prompt_path,
            enable_thinking ? ".template.with-system-and-thinking" : ".template.with-system"
        );
    } else {
    ...
    if (!file) {
        fprintf(stderr, "Couldn't load prompt template %s\n", prompt_path);
        exit(EXIT_FAILURE);
    }
...
void generate(Transformer* transformer, Tokenizer* tokenizer, Sampler* sampler, char* prompt) {
    ...
    while (pos < transformer->config.seq_len) {
        ...
        if (pos >= num_prompt_tokens
            && (next == tokenizer->bos_token_id || next == tokenizer->eos_token_id)) {
            break;
        }
    }
    printf("\n");
    ...
    while (1) {
        ...
        if (user_turn) {
            ...
            if (pos == 0 && system_prompt) {
                sprintf(
                    rendered_prompt, tokenizer->system_prompt_template, system_prompt, user_prompt
                );
            } else {
